...
export default function DailyTestPage({ QuizId }) {
  ...
  return (
    <div className="py-[6rem] lg:py-[8rem]">
      <div className="mx-6">
        <h1 className="text-center font-bold text-[2rem] md:text-[2.5rem] mb-6" dangerouslySetInnerHTML={{ __html: decodeHtmlEntities(QuizId.name) }}></h1>
        <div className="md:mx-12 my-1">
        <Image
              width={500}
              height={500}
              alt="logo"
              src={`${process.env.NEXT_PUBLIC_BACKEND_URL_IMAGE}/img/dailytests/${QuizId.photo}`}
              className="w-full mx-auto rounded-lg"
            />
        </div>
        <p className="mt-4 text-justify text-lg " dangerouslySetInnerHTML={{ __html: decodeHtmlEntities(QuizId.description) }} />
        <AdBanner data-ad-slot="1848801465" data-ad-format="auto" data-full-width-responsive="true" />

        {QuizId.data.length > 0 && (
          <div>
            ...
            <div className="faq-container rounded-lg mt-4 w-[100%]">
              {QuizId.data.map((question, index) => (
      ...
      {role && <PatchDailyTest details={QuizId} />}
    </div>
  );
}
